...
pipeline {
  ...
  stages {
    ...
    stage("Build image") {
      steps {
        script {
          myapp = docker.build("env.CONTAINER_REGISTRY/evn.GOOGLE_CLOUD_PROJECT/petclinic:${env.BUILD_ID}")        }
      }
    }
  }
}
...
Los pasos para el despliegue de la nueva imagen del contenedor actualizada consistirán:
