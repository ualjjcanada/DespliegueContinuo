...
Para ello, debes generar una nueva pareja de claves que usaremos para el despliegue, utilizando el comando ssh-keygen. Realiza los siguientes pasos: 
...
[start=2]
. Crea la carpeta donde se van a guardar la pareja de claves: `mkdir /home/ubuntu/jenkins_home/.ssh`
...
[start=14]
. La primera vez que realizas una conexión ssh desde un usuario en una máquina origen a una destino, te pregunta si deseas almacenar la clave de host de de destino en la lista de máquinas conocidas (`known_hosts`) de tu máquina origen. Contesta: `yes`

.Validar la clave del host: *yes*
image::ssh-host-autentication.png[role="thumb", align="center"]

[start=15]
. Si todo ha ido bien, la conexión se ha debido realizar. Sal con `exit`.
